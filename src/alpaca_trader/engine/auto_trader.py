...
class AutoTrader:
    ...
    async def run_cycle(self) -> dict:
        ...
        try:
            signals = self.scanner.scan(symbols, strategy="bb_rsi_reversal")
            actionable = [s for s in signals if s.detected]
            summary["signals_found"] = len(actionable)
        except Exception as e:
            summary["errors"].append(f"Scanner failed: {e}")
            logger.error("AutoTrader: scanner failed: %s", e)
            return summary
