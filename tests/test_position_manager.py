...
class TestStopLoss:
    def test_stop_loss_triggers(self):
        ...
        should, reason = pm.should_exit(
            "AAPL", current_price=70.0, entry_price=100.0, entry_time=recent_entry()
        )
    ...
    def test_stop_loss_just_above_threshold(self):
        pm = make_pm(stop_loss_pct=-0.30)
        should, _ = pm.should_exit(
            "AAPL", current_price=71.0, entry_price=100.0, entry_time=recent_entry()
        )
...
class TestTakeProfit:
    def test_take_profit_triggers(self):
        ...
        should, reason = pm.should_exit(
            "AAPL", current_price=151.0, entry_price=100.0, entry_time=recent_entry()
        )
        assert should is True
    ...
    def test_take_profit_at_exact_threshold(self):
        pm = make_pm(take_profit_pct=0.50)
        should, reason = pm.should_exit(
            "AAPL", current_price=150.0, entry_price=100.0, entry_time=recent_entry()
        )
        assert should is True
    ...
    def test_take_profit_just_below_threshold(self):
        pm = make_pm(take_profit_pct=0.50)
        should, _ = pm.should_exit(
            "AAPL", current_price=149.0, entry_price=100.0, entry_time=recent_entry()
        )
        assert should is False
...
class TestTrailingStop:
    def test_trailing_stop_not_active_below_trigger(self):
        ...
        should, _ = pm.should_exit(
            "AAPL", current_price=110.0, entry_price=100.0, entry_time=recent_entry()
        )
        assert should is False
    ...
    def test_trailing_stop_activates_and_triggers(self):
        ...
        pm.should_exit(
            "TSLA", current_price=130.0, entry_price=100.0, entry_time=recent_entry()
        )
        # Second call: price drops to 130*(1-0.15)=110.5 (should trigger)
        should, reason = pm.should_exit(
            "TSLA", current_price=110.0, entry_price=100.0, entry_time=recent_entry()
        )
        assert should is True
    ...
    def test_trailing_stop_updates_high_water_mark(self):
        pm = make_pm(trailing_stop_trigger=0.25, trailing_stop_pct=0.15)
        pm.should_exit(
            "MSFT", current_price=130.0, entry_price=100.0, entry_time=recent_entry()
        )
        pm.should_exit(
            "MSFT", current_price=140.0, entry_price=100.0, entry_time=recent_entry()
        )
    ...
    def test_trailing_stop_not_triggered_price_above_trail(self):
        pm = make_pm(trailing_stop_trigger=0.25, trailing_stop_pct=0.15)
        pm.should_exit(
            "AAPL", current_price=130.0, entry_price=100.0, entry_time=recent_entry()
        )
        # Price still above trail (130 * 0.85 = 110.5, current=125 > 110.5)
        should, _ = pm.should_exit(
            "AAPL", current_price=125.0, entry_price=100.0, entry_time=recent_entry()
        )
        assert should is False
...
class TestTimeBasedExit:
    def test_max_hold_days_triggers(self):
        ...
        should, reason = pm.should_exit(
            "AAPL", current_price=100.0, entry_price=100.0, entry_time=old_entry(35)
        )
        assert should is True
    ...
    def test_within_hold_days_ok(self):
        pm = make_pm(max_hold_days=30)
        should, _ = pm.should_exit(
            "AAPL", current_price=100.0, entry_price=100.0, entry_time=recent_entry()
        )
        assert should is False
    ...
    def test_invalid_entry_time_handled(self):
        ...
        # Should not raise
        should, _ = pm.should_exit(
            "AAPL", current_price=100.0, entry_price=100.0, entry_time="not-a-date"
        )
...
class TestInvalidInputs:
    def test_zero_entry_price_returns_false(self):
        pm = make_pm()
        should, _ = pm.should_exit(
            "AAPL", current_price=100.0, entry_price=0.0, entry_time=recent_entry()
        )
        assert should is False
