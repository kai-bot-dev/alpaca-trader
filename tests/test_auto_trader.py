...
def make_signal(symbol, strategy='bb_rsi_reversal',
                detected=True, strength=0.8,
                direction='bullish'):
    """Create a Signal for testing."""
    return Signal(
        symbol=symbol, strategy=strategy, detected=detected,
...
class TestAutoTraderPipeline:
    ...
    def _setup_trader(self, tmp_path, dry_run=True, risk_config=None):
        ...
        rm = RiskManager(risk_config or RiskConfig(
            max_position_pct=0.10, max_open_positions=5,
            min_cash_reserve_pct=0.10, max_order_value=50_000.0,
            min_order_value=10.0, max_trades_per_day=10,
        ))
        executor = OrderExecutor(config=ExecutorConfig(dry_run=dry_run, retry_delay_ms=0))
        pm = PositionManager()
        scanner = MagicMock()
        trader = AutoTrader(
            risk_manager=rm, order_executor=executor,
    ...
    def _mock_account_and_market(self):
        ...
        def mock_bars(symbol, period="1D", limit=2):
            return pd.DataFrame({
                'open': [148.0, 149.0], 'high': [152.0, 153.0],
                'low': [147.0, 148.0], 'close': [150.0, 151.0],
                'volume': [1000000, 1100000],
            })

        return {
            'db': patch('alpaca_trader.engine.auto_trader.db'),
            'market': patch('alpaca_trader.engine.auto_trader.is_market_open', return_value=True),
            'account': patch.object(alpaca_client, 'get_account', return_value={
                'portfolio_value': '100000.0', 'cash': '50000.0', 'equity': '100000.0',
            }),
            'positions': patch.object(alpaca_client, 'get_positions', return_value=[]),
            'bars': patch.object(alpaca_client, 'get_stock_bars_df', side_effect=mock_bars),
        }
    ...
    def test_single_signal_executes_entry(self, tmp_path):
        ...
        with patches['db'] as mock_db, patches['market'],              patches['account'], patches['positions'], patches['bars']:
            mock_db.setting_get = AsyncMock(return_value='true')
    ...
    def test_risk_rejection_blocks_entry(self, tmp_path):
        risk_cfg = RiskConfig(max_open_positions=1, max_position_pct=0.10,
                              min_cash_reserve_pct=0.10, max_order_value=50_000.0,
                              min_order_value=10.0, max_trades_per_day=10)
        ...
        import alpaca_trader.core.client as alpaca_client
        existing = [{'symbol': 'AAPL', 'current_price': '150.0',
                     'avg_entry_price': '140.0', 'qty': '10'}]

        with patches['db'] as mock_db, patches['market'],              patches['account'],              patch.object(alpaca_client, 'get_positions', return_value=existing),              patches['bars']:
            mock_db.setting_get = AsyncMock(return_value='true')
    ...
    def test_skip_already_held_symbol(self, tmp_path):
        ...
        import alpaca_trader.core.client as alpaca_client
        existing = [{'symbol': 'AAPL', 'current_price': '155.0',
                     'avg_entry_price': '145.0', 'qty': '10'}]
...
class TestAutoTraderExitPipeline:
    def test_stop_loss_exit_executes(self, tmp_path):
        ...
        run(journal.log_entry(symbol='AAPL', side='buy', qty=10, price=100.0,
                              strategy='bb_rsi_reversal', signal_details={}))

        import alpaca_trader.core.client as alpaca_client
        positions = [{'symbol': 'AAPL', 'current_price': '65.0',
                      'avg_entry_price': '100.0', 'qty': '10'}]

        with patch('alpaca_trader.engine.auto_trader.db') as mock_db,              patch('alpaca_trader.engine.auto_trader.is_market_open', return_value=True),              patch.object(alpaca_client, 'get_account', return_value={
                 'portfolio_value': '100000.0', 'cash': '50000.0', 'equity': '100000.0',
             }),              patch.object(alpaca_client, 'get_positions', return_value=positions),              patch.object(alpaca_client, 'get_stock_bars_df', return_value=MagicMock(empty=True)):
            mock_db.setting_get = AsyncMock(return_value='true')
            mock_db.watchlist_list = AsyncMock(return_value=[])
            summary = run(trader.run_cycle())
    ...
    def test_healthy_position_not_exited(self, tmp_path):
        ...
        trader = AutoTrader(position_manager=pm, order_executor=executor,
                            trade_journal=journal, dry_run=True)
        run(journal.log_entry(symbol='AAPL', side='buy', qty=10, price=100.0,
                              strategy='bounce', signal_details={}))

        import alpaca_trader.core.client as alpaca_client
        positions = [{'symbol': 'AAPL', 'current_price': '110.0',
                      'avg_entry_price': '100.0', 'qty': '10'}]
    ...
    def test_take_profit_exit_executes(self, tmp_path):
        ...
        trader = AutoTrader(position_manager=pm, order_executor=executor,
                            trade_journal=journal, dry_run=True)
        run(journal.log_entry(symbol='TSLA', side='buy', qty=5, price=200.0,
                              strategy='squeeze', signal_details={}))

        import alpaca_trader.core.client as alpaca_client
        positions = [{'symbol': 'TSLA', 'current_price': '310.0',
                      'avg_entry_price': '200.0', 'qty': '5'}]
...
class TestAutoTraderDailyLimit:
    def test_daily_trade_limit_blocks_further_entries(self, tmp_path):
        risk_cfg = RiskConfig(max_trades_per_day=1, max_position_pct=0.10,
                              min_cash_reserve_pct=0.10, max_order_value=50_000.0,
                              min_order_value=10.0, max_open_positions=10)
        journal = make_mock_journal(tmp_path)
        rm = RiskManager(risk_cfg)
        executor = OrderExecutor(config=ExecutorConfig(dry_run=True, retry_delay_ms=0))
        scanner = MagicMock()
        trader = AutoTrader(risk_manager=rm, order_executor=executor,
                            trade_journal=journal, scanner=scanner, dry_run=True)
        ...
        with patch('alpaca_trader.engine.auto_trader.db') as mock_db,              patch('alpaca_trader.engine.auto_trader.is_market_open', return_value=True),              patch.object(alpaca_client, 'get_account', return_value={
                 'portfolio_value': '100000.0', 'cash': '50000.0', 'equity': '100000.0',
             }),              patch.object(alpaca_client, 'get_positions', return_value=[]),              patch.object(alpaca_client, 'get_stock_bars_df', side_effect=mock_bars):
            mock_db.setting_get = AsyncMock(return_value='true')
            mock_db.watchlist_list = AsyncMock(return_value=[{'symbol': 'AAPL'}])
            summary = run(trader.run_cycle())
