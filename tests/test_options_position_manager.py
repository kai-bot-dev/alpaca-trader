...
class TestTimeStop:
    def test_triggers_when_dte_below_threshold(self):
        ...
        should, reason = opm.should_exit(
            "OPT", current_price=2.0, premium_paid=2.0, expiry_date=expiry_in(2)
        )
    ...
    def test_does_not_trigger_when_dte_at_threshold(self):
        opm = make_opm(time_stop_dte=3)
        should, _ = opm.should_exit(
            "OPT", current_price=2.0, premium_paid=2.0, expiry_date=expiry_in(3)
        )
    ...
    def test_does_not_trigger_with_plenty_of_dte(self):
        opm = make_opm(time_stop_dte=3)
        should, _ = opm.should_exit(
            "OPT", current_price=2.0, premium_paid=2.0, expiry_date=expiry_in(10)
        )
        assert should is False
...
class TestHardStop:
    def test_triggers_at_40pct_loss(self):
        ...
        should, reason = opm.should_exit(
            "OPT", current_price=1.75, premium_paid=3.00, expiry_date=expiry_in(10)
        )
        assert should is True
    ...
    def test_triggers_below_40pct_loss(self):
        opm = make_opm(hard_stop_pct=-0.40)
        should, reason = opm.should_exit(
            "OPT", current_price=1.50, premium_paid=3.00, expiry_date=expiry_in(10)
        )
        assert should is True
        assert "hard_stop" in reason
    ...
    def test_does_not_trigger_just_above_threshold(self):
        ...
        # -39%: 3.00 * 0.61 = 1.83
        should, _ = opm.should_exit(
            "OPT", current_price=1.83, premium_paid=3.00, expiry_date=expiry_in(10)
        )
        assert should is False
...
class TestTrailingStop:
    def test_trailing_not_active_below_trigger(self):
        ...
        should, _ = opm.should_exit(
            "OPT", current_price=3.60, premium_paid=3.00, expiry_date=expiry_in(10)
        )
        assert should is False
    ...
    def test_trailing_activates_and_triggers(self):
        ...
        opm.should_exit(
            "OPT", current_price=4.00, premium_paid=3.00, expiry_date=expiry_in(10)
        )
        # Second: price drops to 4.00*(1-0.20)=3.20 → triggered
        should, reason = opm.should_exit(
            "OPT", current_price=3.20, premium_paid=3.00, expiry_date=expiry_in(10)
        )
        assert should is True
    ...
    def test_trailing_not_triggered_price_above_trail(self):
        ...
        should, _ = opm.should_exit(
            "OPT", current_price=3.50, premium_paid=3.00, expiry_date=expiry_in(10)
        )
        assert should is False
    ...
    def test_high_water_mark_tracked_per_symbol(self):
        opm = make_opm()
        opm.should_exit(
            "OPT1", current_price=5.0, premium_paid=3.0, expiry_date=expiry_in(10)
        )
        opm.should_exit(
            "OPT1", current_price=6.0, premium_paid=3.0, expiry_date=expiry_in(10)
        )
        opm.should_exit(
            "OPT1", current_price=4.5, premium_paid=3.0, expiry_date=expiry_in(10)
        )
...
class TestTakeProfit:
    def test_triggers_at_50pct_gain(self):
        ...
        should, reason = opm.should_exit(
            "OPT", current_price=4.50, premium_paid=3.00, expiry_date=expiry_in(10)
        )
        assert should is True
    ...
    def test_does_not_trigger_just_below_50pct(self):
        opm = make_opm(take_profit_pct=0.50)
        should, _ = opm.should_exit(
            "OPT", current_price=4.49, premium_paid=3.00, expiry_date=expiry_in(10)
        )
        assert should is False
    ...
    def test_triggers_above_50pct(self):
        opm = make_opm(take_profit_pct=0.50)
        should, reason = opm.should_exit(
            "OPT", current_price=5.00, premium_paid=3.00, expiry_date=expiry_in(10)
        )
        assert should is True
        assert "take_profit" in reason
...
class TestExitPriority:
    def test_time_stop_takes_priority_over_profit(self):
        ...
        should, reason = opm.should_exit(
            "OPT", current_price=6.00, premium_paid=3.00, expiry_date=expiry_in(2)
        )
        assert should is True
        assert "time_stop" in reason
    ...
    def test_hard_stop_takes_priority_over_trailing(self):
        opm = make_opm(
            hard_stop_pct=-0.40, trailing_trigger_pct=0.30, trailing_stop_pct=0.20
        )
...
class TestInvalidInputs:
    ...
    def test_no_expiry_skips_time_stop(self):
        ...
        should, reason = opm.should_exit(
            "OPT", current_price=3.0, premium_paid=3.0, expiry_date=None
        )
        assert should is False
        assert reason == "hold"
...
class TestCheckExits:
    ...
    def test_time_stop_from_journal_expiry(self):
        ...
        journal = [
            {"option_symbol": "OPT1", "premium_paid": 3.00, "expiry_date": expiry_in(1)}
        ]
        exits = opm.check_exits(positions, journal_entries=journal)
        assert len(exits) == 1
